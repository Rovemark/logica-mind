...
### Added
- **`logica_mind.continuity`** — a continuity layer that turns memory into a mind.
  Six dependency-injected organs on top of the store:
  - **`SelfModel`** — a versioned, hash-chained, restorable agent identity (skills,
    drives, beliefs): a version + a `current` pointer written in one atomic
    `store.add`, compare-and-set on save, EMA merge (no catastrophic forgetting),
    portable across machines. CLI: `python -m logica_mind.continuity.self_model`.
  - **`Heartbeat`** — a background cognitive cycle (perceive → consolidate → connect
    → hypothesize → self-correct → rewrite → emerge) driving a `LogicaMind`;
    falsifiable hypotheses with a check time; fail-soft with no LLM.
  - **`classify_self_change` / `zone_guard` / `SelfRewriteBlocked`** +
    `SelfModel.detect_drift` — a constitutional brake on self-edits (green/yellow/red).
  - **`WorldInsights`** — a shared cortex (publish / `top_for`, visibility-scoped,
    deduped, prompt-injection-sanitized).
  - **`Metacog`** — `assess` / `who_knows` / `route`: an agent that knows its edges.
  - **`Debate`** — disagreement with epistemic consequence.
  - **`CausalModel`** — a heuristic "simulate before acting" scaffold (not a learned
    world-model; says so on every call).
- HTTP endpoints: `GET`/`POST /api/self-model`, `POST /api/heartbeat/beat`.
- 42 offline tests for the continuity layer.
