...
import { api, ALL, type AddResult } from "../api";
import { useI18n } from "../i18n";
import LearningBurst from "./LearningBurst";

// Write actions from the UI: add a durable memory or a user observation into the
// selected namespace. After a write, the modal animates what the engine LEARNED
// (extracted facts, updates, links) instead of just closing.
export default function Composer({ ns, onDone }: { ns: string; onDone: () => void }) {
  ...
  const [learned, setLearned] = useState("");   // the input that produced `result`
  ...
  async function save() {
    ...
    setBusy(true);
    try {
      const r = await api.add(ns, v, kind);
      setLearned(v); setResult(r); onDone();   // refresh counts behind the animation
  ...
  return (
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-[60] bg-black/55 backdrop-blur-[2px] grid place-items-center p-3 sm:p-4" onClick={close}>
          <div className="w-full max-w-[460px] max-h-[90vh] overflow-y-auto card-surface p-4 fadein" onClick={(e) => e.stopPropagation()}>
            {result ? (
              <LearningBurst input={learned} result={result} onDone={close} />
            ) : (
              <>
                <div className="flex items-center gap-2 mb-3 flex-wrap">
                  ...
                  <button onClick={close} className="ml-auto text-[var(--dim)] hover:text-[var(--txt)]"><X size={16} /></button>
                </div>
