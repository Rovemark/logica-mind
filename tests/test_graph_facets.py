...
def test_graph_viz_cache_hits_and_revalidates():
    m = _mind()
    _edge(m, "A", "uses", "B")
    ...
    # Normal reads are stale-while-revalidate so a large graph opens instantly.
    # A forced refresh (the background worker uses the same path) must expose
    # the write and become the new cache value.
    m.graph_viz(namespace="t")
    v3 = m.graph_viz(namespace="t", _force=True)
    assert v3 is not v1
    assert len(v3["nodes"]) > len(v1["nodes"])
    assert m.graph_viz(namespace="t") is v3
